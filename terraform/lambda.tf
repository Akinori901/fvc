...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      # Cognito (認証)
      #
      # 認証は cognito-auth-service（共通基盤）に統合済み。このリポジトリの
      # cognito.tf が作る fvc-user-pool はもう使っていない。
      #
      # 値をハードコードせず変数にしているのは、cognito-auth-service が別リポジトリ
      # （Akinori901/cognito-auth-service）の管理下にあり、ここから参照できないため。
      ...
      # 共通認証基盤（auth-console）。管理者ロールを画面で管理するために使う。
